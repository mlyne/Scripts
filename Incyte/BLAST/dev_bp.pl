...
while (my $result = $blast->next_result)
{
  ...
  while (my $hit = $result->next_hit)
  {
    ...
    while (my $hsp = $hit->next_hsp)
    {
      ...
      $query_end = $hsp->q_end;
      push(@q_coords, $query_beg);
      push(@q_coords, $query_end);

      my $q_coverage = $query_end-$query_beg;
      print "Coverage: $q_coverage\n";
	...
	push(@s_coords, $subj_beg);
	push(@s_coords, $subj_end);
      }
...
#    print "\@s_coverage:", scalar(@s_coverage), "\n";
    ...
    print "First QCo-ord: $q_coords[0]\n";
    print "Last QCo-ord: $q_coords[-1]\n";
    print "Tot_Qcover: $total_qcov\n";
    print "First SCo-ord: $s_coords[0]\n";
    print "Last SCo-ord: $s_coords[-1]\n";
    print "Tot_Scover: $total_scov\n";
