...
use LWP::Simple;
use XML::Twig;
use DBD::ODBC; # Win32::ODBC doesn't allow parameter binding
use Acme::Comment type => 'C++'; # http://www.perl.com/lpt/a/663
...
# Connect to database
my $db = "Whisker_Publications";
my $driver = "ODBC";
print "Connecting to database...\n";
my $dbh = DBI->connect('DBI:'.$driver.':'.$db) or die "Failed to connect to database: $db\n";
our $dbtype = $dbh->get_info(17); # DBI::SQL_DBMS_NAME
print "Connected to database: $driver:$db. Database type is: $dbtype\n";
our $insert_sql = $dbh->prepare("INSERT INTO PubMedArticles" .
                                "           (PMID, Authors, CreationYear, PublicationYear, Title, Journal," .
                                "            Volume, Pages, Abstract, DOI, AuthorsBrief)" .
                                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
our $query_sql = $dbh->prepare("SELECT COUNT(*) FROM PubMedArticles WHERE PMID = ?;");
...
sub insert_row {
	...
	# see http://www.perl.com/pub/a/1999/10/DBI.html
	if ($dbtype =~ /Access/i) {
		$query_sql->bind_param(1, $pmid, DBI::SQL_VARCHAR);
	} else {
		$query_sql->bind_param(1, $pmid);
	}
	$query_sql->execute() or die "Couldn't query database: " . $query_sql->errstr;
	my @data;
	my $count = 0;
	while (@data = $query_sql->fetchrow_array()) {
		$count = $data[0];
	}
	if ($count > 0) {
		print "  - PMID already in database; skipping.\n";
		return;
	}
	
    # send to database
	if ($dbtype =~ /Access/i) {
		$insert_sql->bind_param(1, $pmid, DBI::SQL_VARCHAR);          # refers to the first ? in the query
		$insert_sql->bind_param(2, $authors, DBI::SQL_LONGVARCHAR);       # refers to the second ? in the query
		$insert_sql->bind_param(3, $creationyear, DBI::SQL_VARCHAR);
		$insert_sql->bind_param(4, $publicationyear, DBI::SQL_VARCHAR);
		$insert_sql->bind_param(5, $title, DBI::SQL_LONGVARCHAR);
		$insert_sql->bind_param(6, $journal, DBI::SQL_VARCHAR);
		$insert_sql->bind_param(7, ($volume eq "" ? undef : $volume), DBI::SQL_VARCHAR);
		$insert_sql->bind_param(8, ($pages eq "" ? undef : $pages), DBI::SQL_VARCHAR);
		$insert_sql->bind_param(9, ($abstract eq "" ? undef : $abstract), DBI::SQL_LONGVARCHAR);
		$insert_sql->bind_param(10, ($doi eq "" ? undef : $doi), DBI::SQL_VARCHAR);
		$insert_sql->bind_param(11, $authors, DBI::SQL_LONGVARCHAR);
    } else {
		$insert_sql->bind_param(1, $pmid);          # refers to the first ? in the query
		$insert_sql->bind_param(2, $authors);       # refers to the second ? in the query
		$insert_sql->bind_param(3, $creationyear);
		$insert_sql->bind_param(4, $publicationyear);
		$insert_sql->bind_param(5, $title);
		$insert_sql->bind_param(6, $journal);
		$insert_sql->bind_param(7, ($volume eq "" ? undef : $volume) );
		$insert_sql->bind_param(8, ($pages eq "" ? undef : $pages) );
		$insert_sql->bind_param(9, ($abstract eq "" ? undef : $abstract) );
		$insert_sql->bind_param(10, ($doi eq "" ? undef : $doi) );
		$insert_sql->bind_param(11, $authors);
    }
