...
#!#	my ($country_h) = ($country_header =~ m/^\(*(.+?)\[.+$/) ? $1 : $country_header;
...
#	my ($indicName) = ($indicExp =~ m/^\(*(.+?)\[.+$/) ? $1 : $indicExp;
	my $indicName = $indID;
	print $indicName, "\n";
	
	READ_SCOPE_LOOP:while (<SFILE>)
	{
...
# Start at row #2 as row #1 has country headers
	...
	READ_QUERY_LOOP: for my $entry (@country)
	{
...
# 
#         my $url = "http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?&db=pubmed&rettype=count&term=$indicExp" . # test URL 2
...
#         $response->is_success or print "$indicName, $entry\tError: " . 
