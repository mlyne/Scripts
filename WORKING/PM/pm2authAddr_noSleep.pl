...
#    print "$total $rangeCnt $cutoff\n"; # print for testing
  ...
  if (@highHits) {
    foreach my $topAuth (@highHits) {
      print OUT_FILE $id, "\t", $authCount{$topAuth}, "\t", $topAuth, "\tADDR: ", join(': ', @{ $authAddr{$topAuth} } ), "\n";
    }
  } else {
      foreach my $key (sort { $authCount {$b} <=> $authCount {$a} } keys %authCount) 
      {
	print OUT_FILE $id, "\t", $authCount{$key}, "\t", $key, "\tADDR: ", join(': ', @{ $authAddr{$key} } ), "\n";
      }
  }
...
#   foreach my $key (sort { $authCount {$b} <=> $authCount {$a} } keys %authCount) 
#   {
#     print OUT_FILE $authCount{$key}, " ", $key, "\tADDR:", $authAddr{$key}, "\n";
#   }
  
  print "--- Finished for $id ---\n\n";
...
#     my $addrCnt =  @{ $authAddr{$topAuth} };
#     
#     if ($addrCnt > 1) { 
#       $addr = @{ $authAddr{$topAuth} }[0];
#       } else {
#       $addr = join(': ', @{ $authAddr{$topAuth} } );
#       }
#     print "ADDR COUNT  ", $addrCnt, "\n";
...
#      my $addr = $article->first_descendant('Affiliation')->text;
    ...
    unless ( (exists $authAddr{$author_data}) && ($authAddr{$author_data} eq $addr) ) {
#      print "\n\nHere's One!\n\n";
      push @{ $authAddr{$author_data} }, "$addr";
#      print "STUFF: ", $authAddr{$author_data}, "\n";
    }
