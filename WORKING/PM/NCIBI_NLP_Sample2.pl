...
my $response = $ua->get('http://nlp.ncibi.org/fetch?pmid=17523140&tagger=nametagger&type=gene');

my $xp = XML::XPath->new(xml => $response->content);

foreach my $sentence ($xp->find('//Gene[@id=\'11137\']/parent::node()')->get_nodelist) {
	print $sentence->string_value . "\n";
}