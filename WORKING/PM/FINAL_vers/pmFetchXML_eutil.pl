...
my $usage = "Usage:pubFetchXML termsRE.txt\n
\n";
...
$params{maxdate} = "2012/11/01";
$params{email} = "mlyne.careers\@gmail.com";
$params{db} = "pubmed";
$params{mindate} = "2008/01/01";
$params{tool} = "ebot";
$params{datetype} = "PDAT";
$params{term} = "$terms";
#  $params{term} = "glaucoma\[tiab\]+AND+\(trabeculectomy\[tiab\]+OR+filtration\[tiab\]\)";

#$params{term} = "glaucoma";
%params = esearch(%params);

$params{retmode} = "xml";
$params{outfile} = "$id" . ".xml";
efetch_batch(%params);
}
...
my (%sparams, %lparams, %ct, %pparams, %posted, %iparams, %hparams);
...
#         print "Links failed. Trying again...\n";
   ...
   foreach my $key (keys %{$lresults{to}}) { 
     $foundnames{$key}{ct}++; 
...
my (%sparams, %lparams, %iparams, %iresults, %lresults, %pparams);
my $key;
...
# use esearch to retrieve each batch of input UIDs, and then use elink_history
   ...
   #post the HTTP request
   my $raw = $ua->get($linkurl, ':content_file' => $tempfile);
