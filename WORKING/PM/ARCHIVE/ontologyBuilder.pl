...
$big_string =~ s/ -+\w+/ /g;	# words starting with a dash
$big_string =~ s/ -+\d+/ /g;	# numbers starting with a dash
$big_string =~ s/ \w / /g;	# free singlet characters
$big_string =~ s/ \d+ / /g;	# free digits
$big_string =~ s/\s+/ /g;	# multiple whitespaces
$big_string =~ s/ \w\w / /g;	# free doublet characters
$big_string =~ s/ \d+ / /g;	# free multi digets
...
#my %allWordCnt = (%singleWordCnt, %doubleWordCnt, %tripleWordCnt, %fourWordCnt);
