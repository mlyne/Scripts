...
while (<QFILE>)
{
  ...
  my ($searchStr, $inVal) = split(/\t/, $_);  ### changed
  my $query = "$searchStr";
...
sub ops_post {
  ...
  my $range = shift; ### change line
#  my $range = defined $inVal ? ($inVal) : ("1-25"); ### changed code
...
#  my $twig = new XML::Twig( twig_handlers => { 'exchange-document' => \&process_patRes });
  my $twig = new XML::Twig( twig_handlers => { 'exchange-document' => \&process });
  $twig->parse( $response );
...
sub process
{
  ...
  {
    print "CHILD_NAME: ", $child->name,": CHILD_TEXT: ", $child->text,"\n";
  }
}
...
### WORKING_AREA ###
#  my @docIds = $entry->children('//publication-reference');
  
#   foreach my $idRef (@docIds) {
#      print $idRef->first_child()->text, "\n\n";
#      print $idRef->gi(), "\n\n";
#     my $epoIdRef = $idRef->first_child('document-id[@document-id-type="epodoc"]') if $idRef;
#     my $docNo = $epoIdRef->first_child('doc-number')->text  if $epoIdRef;
#     my $docDate = $epoIdRef->first_child('date')->text  if $epoIdRef;
#     print "Document: ", $docNo, "\t:\t$docDate\n" if ($docNo && $docDate);
