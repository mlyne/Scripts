...
die "Usage: transc2GTF.pl transcriptID_file\n" unless @ARGV;
...
while (<INFILE>) {   
  ...
  $_=~ s/\>//; # remove leading
  my $seqname = $_;
  my $source = "transcriptome";
  my $feature = "exon";
  my $start = "1";

  my ($locus, $transc, $len);
  $_ =~ /(Locus_\d+)_(Transcript_.+?_Length)_(\d+)/;
  $locus = $1;
  $transc = $2;
  $len = $3;
  ...
  my $end = "$len";
  my $fill = "\.\t\+\t\.";
  my $geneID = "gene_id \"$locus\"\; ";
  my $transcID = "transcript_id \"$seqname\"\;";
  my $gene2trans = "$geneID$transcID";

  print $seqname, "\t", $source, "\t", $feature, "\t", $start, "\t", $end .
    "\t", $fill, "\t", $gene2trans, "\n";
  
#  print "NOW: ", $_, "\n";
#  print "$locus, $transc, $len, $fill\n";
