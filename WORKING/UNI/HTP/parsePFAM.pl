...
#  $refSize++ if @PFAMlist; # count how many reference results have PFAM terms
  my $grepCnt = grep /PF\d+:/, @PFAMlist;
  $refSize += $grepCnt if $grepCnt; # count how many reference results have PFAM terms
...
#  next unless $_ =~ /ZDB-GENE/; # remove for full analysis
  ...
  my $grepCnt = grep /PF\d+:/, @PFAMlist;
  $sampSize += $grepCnt if $grepCnt; # count how many sample results have PFAM terms
