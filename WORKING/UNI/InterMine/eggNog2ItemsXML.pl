...
my $usage = "eggNog2ItemsXML.pl <species_set> <taxon_file> <im_model_file.xml>
...
species_set: either bactNOG (bacteria) or arNOG (archaea)

Required files:
Taxon file - format: 
Tax1 Tax2 Tax3 etc

Model file: intermine model.xml file containing the functional categories 
model extension used in synbio-funccat (see synbio-funccat_additions)
Copy located in: /data/eggnog/Model/

";

my ($spec_set, $taxon_file, $model_file) = @ARGV;
unless ( $ARGV[2] ) { die $usage }

### Files to process
#my $base_url = "/home/ml590/MIKE/InterMine/SynBioMine/DataSources/EggNog/";
my $base_url = "/home/ml590/MIKE/InterMine/ArchaeaMine/DataSources/EggNog/";
...
my $nog_members = $base_url . $spec_set . ".members.txt";
my $nog_funccat = $base_url . $spec_set . ".funccat.txt";
my $nog_description = $base_url . $spec_set . ".description.txt";
