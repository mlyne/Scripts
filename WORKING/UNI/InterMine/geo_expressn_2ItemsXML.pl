...
#  print "VALS: $mean, $log_fold_change, $cv \n";
    ...
    if (exists $seen_genes{$gene}) {
      $result_item->set( gene => $seen_genes{$gene} );
    } else {
      $result_item->set( gene => $gene_item );
    }
#    $result_item->set( gene => $gene_item );
    $seen_genes{$gene} = $gene_item unless (exists $seen_genes{$gene});
  ...
  if (exists $seen_genes{$gene}) {
    my $seen_gene_item = $seen_genes{$gene};
    $seen_gene_item->set( expressionResults => \@results_items );
  } else {
