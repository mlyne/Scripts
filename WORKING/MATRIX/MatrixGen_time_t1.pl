...
# Make use of LWP to make call to PubMed query website
        ...
        #my $url = "http://eutils.ncbi.nlm.nih.gov/entrez/eutils/egquery.fcgi?term=$drugExp" . # test URL
        ...
        sleep 3; # PubMed requires that we leave at least 3 seconds between requests
...
# feed into $entry
		...
		my ($hit) = grep /PubMed/, @res;
		my ($hitCount) = defined $valid ? ($hit =~ m/\<Count\>(\d+)\</) : ("12321");
